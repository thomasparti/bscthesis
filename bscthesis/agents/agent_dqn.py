...
def agent(cfg: str, model: str, logdir: str, video: Optional[str], episodes: int = 10, delay: float = 0.0, render: bool = False):
    ...
    if video:
        ...
        if temp_frame is not None:
            ...
            if is_color:
                video_writer = cv2.VideoWriter(video, fourcc, 30, (width, height))
            else:
                video_writer = cv2.VideoWriter(video, fourcc, 30, (width, height), isColor=False)
    ...
    try:
        ...
        sys.exit(1)
    print(f"DQN model loaded from '{model}'.")

    print(f"Running {episodes} episodes with the DQN agent...")
